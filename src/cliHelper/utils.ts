...
const nodeVersions: string[] = ['12', '14', '16'];
 ...
 * @returns credentials
 */
export const getCrossAccountCredentials = async (awsConfig: JSONObject) => {
  return new Promise((resolve, reject) => {
    if (!awsConfig.AWS_ROLE_ARN || (awsConfig.AWS_ROLE_ARN && awsConfig.AWS_ROLE_ARN.length == 0)) {
      ...
      const sts: JSONObject = new AWS.STS({
        region: awsConfig.AWS_REGION,
        accessKeyId: awsConfig.AWS_ACCESS_KEY_ID,
        secretAccessKey: awsConfig.AWS_SECRET_ACCESS_KEY_ID,
      });
      ...
      sts.assumeRole(params, (err: string, data: JSONObject) => {
        if (err) reject(err);
        else {
          resolve({
            region: awsConfig.AWS_REGION,
...
export const validatePackageVersion = (val: JSONObject | string) => {
...
export const checkAndFindVersion = (CLI_ARGS: string) => {
  return CLI_ARGS.includes("version") ? CLI_ARGS[CLI_ARGS.indexOf("version") + 1] ?? false : false;
}

export const updateNodeModulesStructure = async (pluginName: string) => {
  const spinner = ora(`Copying files to root folder ... `);
  try {
    ...
    let fileList: string[] = await fs.promises.readdir(pluginPath);
    ...
    const folderNames: string[] = await fs.promises.readdir(path.resolve(`./node_modules/${pluginName}`))
...
export const updateSrcFiles = async (folderName: string) => {

  const pluginPackageJson: JSONObject = JSON.parse(
